...
module PackAPI::Mapping
  RSpec.describe PackAPI::Querying::ComposableQuery, type: :model do
    describe '#add' do
      ...
      context 'with composer = :merge' do
        let(:query) { described_class.new(BlogPost.all, composer: :merge) }
      ...
      context 'with composer = :and' do
        let(:query) { described_class.new(BlogPost.all, composer: :and) }
